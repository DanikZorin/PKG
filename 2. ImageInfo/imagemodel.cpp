        ...
        for (const QString &path : paths) {
            ...
            // Остальные форматы
                ...
                if (info.format == "JPG" || info.format == "JPEG")
                    ...
                    info.compression = "Uncompressed / RLE";
                else
                    info.compression = "N/A";
            }
            info.compression += QString(" %1\%").arg(100*info.fileSize/(info.dimensions.width()*info.dimensions.height()*info.depth/8));
...
void ImageModel::sort(int col, Qt::SortOrder order){
    if (col == 0){ /// File name
    ...
    if (col == 2){ /// File size Px
        std::sort(images.begin(), images.end(),
            [](const ImageInfo& a, const ImageInfo& b){
              return a.dimensions.height()*a.dimensions.width()<b.dimensions.height()*b.dimensions.width();
            });
    }
    if (col == 3){ /// File res
        std::sort(images.begin(), images.end(),
            [](const ImageInfo& a, const ImageInfo& b){
              return a.dpiX*a.dpiY<b.dpiX*b.dpiY;
            });
    }
    if (col == 4){ /// File color depth
        std::sort(images.begin(), images.end(),
            [](const ImageInfo& a, const ImageInfo& b){
              return a.depth<b.depth;
            });
    }
    if (col == 5){ /// File format
        std::sort(images.begin(), images.end(),
            [](const ImageInfo& a, const ImageInfo& b){
                return a.format<b.format;
            });
    }
    if (col == 6){ /// File compress
        std::sort(images.begin(), images.end(),
            [](const ImageInfo& a, const ImageInfo& b){
